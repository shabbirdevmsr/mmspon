# ==========================================================
# a.py — multi-API Telegram bot with 2GB uploads + .env config
...
def ensure_defaults():
    if api_col.count_documents({}) == 0:
        api_col.insert_one({
            "_id": "api1",
            "name": "DesiTube (mmsbaba)",
    ...
    tracking_col.update_one(
        {"_id": "api1"},
        {"$setOnInsert": {"processed_slugs": [], "in_progress_slugs": []}},
        upsert=True,
    )
    adminstate_col.update_one({"_id": "state"},
...
def get_api(api_id: str) -> dict:
    return api_col.find_one({"_id": api_id}) or {}


def get_all_apis() -> list:
    return list(api_col.find())


def get_targets(api_id: str) -> list:
    return list(target_col.find({"api_id": api_id}))
...
def get_state() -> str:
    return (adminstate_col.find_one({"_id": "state"}) or {}).get("value", "")


def set_state(v: str):
    adminstate_col.update_one({"_id": "state"}, {"$set": {"value": v}}, upsert=True)


def get_welcome(chat_id: int) -> dict:
    return welcome_col.find_one({"_id": chat_id}) or {}
...
async def send_video_with_progress(bot, chat_id, video_path, thumb_path, caption,
    ...
    try:
        await bot.send_video(
            chat_id=chat_id,
            video=InputFile(reader, filename=video_path.name),
...
def extract_status_change(chat_member_update):
    ...
    was_member = old_status in (ChatMember.MEMBER, ChatMember.OWNER,
                                ChatMember.ADMINISTRATOR) or \
                 (old_status == ChatMember.RESTRICTED and old_is_member is True)
    is_member = new_status in (ChatMember.MEMBER, ChatMember.OWNER,
                               ChatMember.ADMINISTRATOR) or \
...
def hidden_keyboard() -> ReplyKeyboardMarkup:
...
async def feat_status(update, ctx, state):
    ...
    for a in apis:
        ...
        targets = get_targets(a["_id"])
        lines.append(
            f"<b>{esc(a['name'])}</b>\n"
            f"  ⚙️ Mode: {a.get('mode','ON')}\n"
            f"  📄 Page: {a.get('current_page', 1)}\n"
            f"  ✅ Processed: {len(t['processed_slugs'])}\n"
...
async def feat_broadcast_send(update, ctx, state):
    ...
    for u in users:
        try:
            await update.message.bot.copy_message(
                chat_id=u["_id"], from_chat_id=update.message.chat.id,
                message_id=update.message.message_id)
            ok += 1
        except Exception as e:
            blog(f"broadcast {u['_id']}: {e}")
...
async def feat_browse_prompt(update, ctx, state):
    ...
    kb = [[InlineKeyboardButton(f"🔌 {a['name']}", callback_data=f"apick_{a['_id']}")]
          for a in apis]
    await update.message.reply_text("Pick an API source:",
...
async def feat_pick_api(update, ctx, state):
    ...
    set_state(f"waiting_page:{api_id}")
    await cq.message.reply_text("🔢 Send a page number.",
        reply_markup=cancel_keyboard())
...
async def feat_browse_page(update, ctx, state):
    ...
    if not data or not data.get("data"):
        await update.message.reply_text(f"❌ No videos on page {p}.",
    ...
    caption = f"🎬 <b>{title}</b>\n\n📄 Page: {p} | Item: 1/{total}\n🆔 <code>{slug}</code>"
    row = [InlineKeyboardButton("⬇️ Download",
        callback_data=f"aget_{api_id}_{p}_0")]
    if total > 1:
        row.append(InlineKeyboardButton("Next ➡️",
            callback_data=f"abrowse_{api_id}_{p}_1"))
    if thumb:
...
async def feat_browse_nav(update, ctx, state):
    ...
    row = []
    if i > 0:
        row.append(InlineKeyboardButton("⬅️ Prev",
            callback_data=f"abrowse_{api_id}_{p}_{i-1}"))
    row.append(InlineKeyboardButton("⬇️ Download",
        callback_data=f"aget_{api_id}_{p}_{i}"))
    if i < total - 1:
        row.append(InlineKeyboardButton("Next ➡️",
            callback_data=f"abrowse_{api_id}_{p}_{i+1}"))
    ...
    if thumb:
        await cq.message.chat.send_photo(photo=thumb, caption=caption,
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup([row]))
    else:
        await cq.message.chat.send_message(caption, parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([row]))
...
async def feat_download(update, ctx, state):
    ...
    tmp = TMP_DIR / f"browse_{int(time.time()*1000)}.mp4"
...
async def feat_run_cron(update, ctx, state):
    ...
    kb = [[InlineKeyboardButton(f"▶️ {a['name']}", callback_data=f"crun_{a['_id']}")]
          for a in apis]
    await update.message.reply_text("Which API to run cron for?",
        reply_markup=InlineKeyboardMarkup(kb))
...
async def feat_cron_pick(update, ctx, state):
    ...
    await run_cron(ctx.bot, cq.message.chat.id, api_id)
...
# CRON PER API
# ==========================================================
async def run_cron(bot, admin_chat_id: int, api_id: str):
    lock = STORAGE / f"cron_{api_id}.lock"
    if lock.exists():
    ...
    try:
        await _cron_pipeline(bot, admin_chat_id, api_id)
    finally:
        try: lock.unlink()
        except FileNotFoundError: pass


async def _cron_pipeline(bot, admin_chat_id: int, api_id: str):
    api = get_api(api_id)
    if not api:
    ...
    if not v:
        ...
        await bot.send_message(admin_chat_id,
            f"✅ Page {page} done → next: {new_page}"); return
    ...
    init_text = (f"⏳ <b>Cron: {esc(api['name'])}</b>\n\n"
                 ...
                 f"⚙️ Mode: {mode}")
    if thumb:
        init = await bot.send_photo(admin_chat_id, photo=open(thumb, "rb"),
            caption=init_text, parse_mode="HTML")
        use_photo = True
    else:
    ...
    if not targets:
        await edit_progress(bot, admin_chat_id, msg_id, use_photo,
            "❌ No channels/groups configured for this API.")
    ...
    if mode == "ON":
        for tg in targets:
            ...
            ok = await send_video_with_progress(
                bot, tg["chat_id"], tmp, t_thumb, cap,
                admin_chat_id, msg_id, use_photo,
                tg["type"], title, slug, page, total, item)
            if ok and tg["type"] == "channel":
                sent_as_video = True
            await asyncio.sleep(1)

    if not sent_as_video:
        await edit_progress(bot, admin_chat_id, msg_id, use_photo,
            f"⚠️ Falling back to photo/link for <code>{slug}</code>")
        for tg in targets:
            try:
                if mode == "OFF" and thumb:
                    await bot.send_photo(tg["chat_id"], photo=open(thumb, "rb"),
                        caption=group_cap, parse_mode="HTML", protect_content=True)
                else:
                    await bot.send_message(tg["chat_id"],
                        f"🎬 <b>{title}</b>\n\n📥 {link}\n\n🔥 @virulvideopompom",
                        parse_mode="HTML", protect_content=True)
            except TelegramError as e:
                blog(f"fallback {tg['chat_id']}: {e}")
            await asyncio.sleep(1)

    cleanup(tmp, thumb, blur)
    finish_slug(api_id, slug, processed=True)
    await edit_progress(bot, admin_chat_id, msg_id, use_photo,
        f"✅ <b>Done!</b>\n\n🎬 <b>{title}</b>\n🆔 <code>{slug}</code>\n"
        f"📄 Page: {page} | Item: {item}/{total}\n"
        f"📤 Sent: {'video' if sent_as_video else 'fallback'}")
...
def register(order, handler, *, buttons=None, callbacks=None,
             admin_only=True, state=None, catch_all=False, state_prefix=None):
    FEATURES.append({
        "order": order, "handler": handler, "buttons": buttons or [],
        "callbacks": callbacks or [], "admin_only": admin_only,
        "state": state, "catch_all": catch_all, "state_prefix": state_prefix,
    })
...
def main():
    ...
    if TG_API_SERVER:
        builder = (
            ...
            .base_file_url(f"{TG_API_SERVER}/file/bot")
        )
        print(f"✓ Using self-hosted Bot API: {TG_API_SERVER}  (2 GB limit)")
    else:
        print("⚠ Using api.telegram.org — uploads limited to 50 MB")
